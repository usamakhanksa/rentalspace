...
class GatewayCurrencyUpdate extends Command
{
    ...
    public function handle()
    {
        if (basicControl()->currency_layer_auto_update == 1) {
            $gateways = Gateway::where('status', 1)->where('currency_type', 1)->get();
            ...
            if (count($gateways) > 0) {
                ...
                if (isset($autoCurrencyUpdate->quotes)) {
                    $quotes = $autoCurrencyUpdate->quotes;
                    foreach ($gateways as $gateway) {
                        try {
                            $receiveArr = [];
                            foreach ($gateway->receivable_currencies as $currency) {
                                if ($source == $currency->name) {
                                    $rate = 1;
                                } else {
                                    $target = $source . '' . $currency->name;
                                    $rate = $quotes->{$target} ?? 1;
                                }

                                $currenciesArr = [];
                                foreach ($currency as $key => $value) {
                                    if ($key == 'conversion_rate') {
                                        $currenciesArr[$key] = $rate;
                                    } else {
                                        $currenciesArr[$key] = $value;
                                    }
                                }
                                $receiveArr[] = $currenciesArr;
                            }

                            $gateway->receivable_currencies = $receiveArr;
                            $gateway->save();
