...
class TransactionLogController extends Controller
{
    public function transaction()
    {
        ...
        $data['transactionStats'] = Transaction::selectRaw("
            COUNT(*) as total,
            SUM(CASE WHEN DATE(created_at) = ? THEN 1 ELSE 0 END) as today,
            SUM(CASE WHEN created_at >= ? THEN 1 ELSE 0 END) as this_week,
            SUM(CASE WHEN created_at >= ? THEN 1 ELSE 0 END) as this_month,
            SUM(CASE WHEN created_at >= ? THEN 1 ELSE 0 END) as this_year
        ", [$today, $startOfWeek, $startOfMonth, $startOfYear])->first();

        $total = $data['transactionStats']->total ?? 1;

        $data['percentages'] = [
            'today' => round(($data['transactionStats']->today / $total) * 100, 2),
            'this_week' => round(($data['transactionStats']->this_week / $total) * 100, 2),
            'this_month' => round(($data['transactionStats']->this_month / $total) * 100, 2),
            'this_year' => round(($data['transactionStats']->this_year / $total) * 100, 2),
        ];
