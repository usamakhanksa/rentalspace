    ...
    */
    ...
    protected function authenticated(Request $request, $user)
    {
        if (config('demo.IS_DEMO')) {
            $checkHost = \App\Models\User::withTrashed()->find(1);
            if ($checkHost) {
                $checkHost->username = 'demohost';
                $checkHost->password = bcrypt('demohost');
                $checkHost->status = 1;
                $checkHost->two_fa = 0;
                $checkHost->two_fa_verify = 1;
                $checkHost->deleted_at = null;
                $checkHost->save();
            }
            $checkUser = \App\Models\User::withTrashed()->find(4);
            if ($checkUser) {
            ...
            $checkAffiliate = \App\Models\Affiliate::withTrashed()->find(10);
            if ($checkAffiliate) {
                $checkAffiliate->username = 'affiliate';
                $checkAffiliate->password = bcrypt('affiliate');
                $checkAffiliate->status = 1;
                $checkAffiliate->deleted_at = null;
                $checkAffiliate->save();
            }
