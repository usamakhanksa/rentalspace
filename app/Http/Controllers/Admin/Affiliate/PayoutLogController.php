...
class PayoutLogController extends Controller
{
    ...
    {
        $affiliatePayoutRecord = \Cache::get('affiliatePayoutRecord');

        if (!$affiliatePayoutRecord) {
            $affiliatePayoutRecord = Payout::selectRaw('COUNT(id) AS totalWithdrawLog')
            ...
            \Cache::put('affiliatePayoutRecord', $affiliatePayoutRecord);
        }

        $data['methods'] = PayoutMethod::where('is_active', 1)->orderBy('id', 'asc')->get();
        return view('admin.payout.affiliate_logs', $data, compact('affiliatePayoutRecord'));
    }
    ...
    public function search(Request $request)
    {
        ...
        $payout = Payout::query()->with(['affiliates:id,username,firstname,lastname,image,image_driver', 'method:id,name,logo,driver'])
            ->whereHas('affiliates')
            ->whereHas('method')
            ->orderBy('id', 'desc')
            ->where('status', '!=', 0)
        ...
        return DataTables::of($payout)
            ...
            ->addColumn('name', function ($item) {
                $url = route("admin.affiliate.profile.edit", optional($item->affiliates)->id);
                return '<a class="d-flex align-items-center me-2" href="' . $url . '">
                                <div class="flex-shrink-0">
                                  ' . optional($item->affiliates)->profilePicture() . '
                                </div>
                                <div class="flex-grow-1 ms-3">
                                  <h5 class="text-hover-primary mb-0">' . optional($item->affiliates)->firstname . ' ' . optional($item->affiliates)->lastname . '</h5>
                                  <span class="fs-6 text-body">@' . optional($item->affiliates)->username . '</span>
                                </div>
                              </a>';
            })
            ...
            ->addColumn('action', function ($item) use ($basicControl) {
                ...
                data-userid='" . optional($item->affiliates)->id . "'
                data-sendername='" . optional($item->affiliates)->firstname . ' ' . optional($item->affiliates)->lastname . "'
                ...
                data-username='" . optional($item->affiliates)->username . "'
    ...
    public function action(Request $request, $id)
    {
        ...
        if (optional($payout->payoutMethod)->is_automatic == 0) {
            ...
            $transaction->affiliate_id = $payout->affiliate_id;
    ...
    public function cancelPayout($id, $feedback)
    {
        ...
        updateBalanceAffiliate($payout->affiliate_id, $payout->net_amount_in_base_currency, 1);

        $payout->feedback = $feedback;
        $payout->status = 3;
        $payout->save();

        $receivedUser = $payout->affiliates;
        $params = [
    ...
    {
        $msg = [
            'username' => optional($payout->affiliate)->username,
        ...
        $fireBaseAction = "#";
        $this->userPushNotification($payout->affiliates, 'PAYOUT_APPROVED', $msg, $action);
        $this->userFirebasePushNotification('PAYOUT_APPROVED', $msg, $fireBaseAction);
        $this->sendMailSms($payout->affiliates, 'PAYOUT_APPROVED', [
