...
class SeoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $id = $seoableClass::where('slug', $request->slug)
            ->where('host_id', auth()->id())
            ->pluck('id')
            ->first();

        if (!$id){
            return back()->with('error', $request->type.' not found.');
        }
        ...
        return view(template()."vendor.seo", $data);
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $id = $seoableClass::where('id', $request->seoable_id)
                ->where('host_id', auth()->id())
                ->pluck('id')
                ->first();

            throw_if(!$id, 'Invalid seoable id');
