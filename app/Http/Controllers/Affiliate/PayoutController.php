...
class PayoutController extends Controller
{
    ...
    public function payout()
    {
        ...
        return view(template() . 'affiliate.payout.request', $data);
    }
    ...
    public function payoutRequest(Request $request)
    {
        try {
            ...
            $payout->status = 0;
            $payout->save();
            return redirect(route('affiliate.payout.confirm', $payout->trx_id))->with('success', 'Payout initiated successfully');
        } catch (\Exception $e) {
    ...
    public function confirmPayout(Request $request, $trx_id)
    {
        ...
        if ($request->isMethod('get')) {
            if ($payoutMethod->code == 'flutterwave') {
                return view(template() . 'affiliate.payout.gateway.' . $payoutMethod->code, compact('payout', 'payoutMethod', 'basic'));
            } elseif ($payoutMethod->code == 'paystack') {
                return view(template() . 'affiliate.payout.gateway.' . $payoutMethod->code, compact('payout', 'payoutMethod', 'basic'));
            }
            return view(template() . 'affiliate.payout.confirm', compact('payout', 'payoutMethod'));
            ...
            if (!updateBalanceAffiliate($payout->affiliate_id, $payout->net_amount_in_base_currency, 0)) {
            ...
            $payout->save();
            return redirect(route('affiliate.payouts'))->with('success', 'Payout generated successfully');
    ...
    public function userNotify($user, $payout)
    {
        ...
        $action = [
            "link" => route('admin.affiliate.payout.log'),
        ...
        $firebaseAction = route('admin.affiliate.payout.log');
        ...
        $action = [
            "link" => route('affiliate.payouts'),
            "icon" => "fa fa-money-bill-alt text-white"
        ];

        $firebaseAction = route('affiliate.payouts');
