...
class SupportTicketController extends Controller
{
    ...
    public function index()
    {
        ...
        $tickets = SupportTicket::where('affiliate_id', auth('affiliate')->id())
            ->latest()
            ->paginate(basicControl()->user_paginate);

        return view(template() . 'affiliate.support_ticket.index', compact('tickets'));
    }

    public function create()
    {
        return view(template() . 'affiliate.support_ticket.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $msg = [
            'user' => optional($ticket->affiliate)->username,
            'ticket_id' => $ticket->ticket
        ];
        $action = [
            "name" => optional($ticket->affiliate)->firstname . ' ' . optional($ticket->affiliate)->lastname,
            "image" => getFile(optional($ticket->affiliate)->image_driver, optional($ticket->affiliate)->image),
        ...
        return redirect()->route('affiliate.ticket.list')->with('success', 'Your ticket has been pending.');
    }
    ...
    public function ticketView($ticketId)
    {
        ...
        $user = Auth::user();
        return view(template() . 'affiliate.support_ticket.view', compact('ticket', 'user'));
    }
    ...
    public function reply(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->reply_ticket == 1) {
                ...
                $msg = [
                    'username' => optional($ticket->affiliate)->username,
                    'ticket_id' => $ticket->ticket
                ];
                $action = [
                    "name" => optional($ticket->affiliate)->firstname . ' ' . optional($ticket->affiliate)->lastname,
