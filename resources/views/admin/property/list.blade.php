...
@section('page_title', __('Properties'))
@section('content')
    <div class="content container-fluid">
        <x-page-header menu="Properties" :statBtn="true"/>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <h6 class="card-subtitle mb-3">@lang('Active Properties')</h6>
                                <h3 class="card-title js-counter" data-value="{{ $propertyStats->active }}">{{ $propertyStats->active }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $propertyStats->total }}</span>
            ...
            <div class="col-sm-6 col-lg-3 mb-3 mb-lg-4">
                <div class="card card-hover-shadow h-100 stats-card">
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <h6 class="card-subtitle mb-3">@lang('Inactive Properties')</h6>
                                <h3 class="card-title js-counter" data-value="{{ $propertyStats->inactive }}">{{ $propertyStats->inactive }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $propertyStats->total }}</span>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                ...
                                <h3 class="card-title js-counter" data-value="{{ $propertyStats->this_month }}">{{ $propertyStats->this_month }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $propertyStats->total }}</span>
            ...
            <div class="col-sm-6 col-lg-3 mb-3 mb-lg-4">
                <div class="card card-hover-shadow h-100 stats-card">
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <h6 class="card-subtitle mb-3">@lang("Created This Year")</h6>
                                <h3 class="card-title js-counter" data-value="{{ $propertyStats->this_year }}">{{ $propertyStats->this_year }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $propertyStats->total }}</span>
            ...
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <div class="input-group input-group-merge input-group-flush">
                        ...
                        <input id="datatableSearch" type="search" class="form-control" placeholder="Search property" aria-label="Search property" autocomplete="off">
                    </div>
                </div>
                            ...
                            <div class="offcanvas-body">
                                <form id="filter_form">
                                    <div class="col-12 mb-4">
                                        <span class="text-cap text-body">@lang("Property Title")</span>
                                        <input type="text" class="form-control" id="filter_input" name="filter_input" placeholder="e.g. Place to stay in srinagar" autocomplete="off">
                                    </div>

                                    <small class="text-cap text-body">@lang("Property Status")</small>
            ...
            <div class=" table-responsive datatable-custom  ">
                <table id="datatable"
                    ...
                    <tr>
                        ...
                        <th scope="col">@lang('Amenities')</th>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="" method="post">
                    @csrf
                    <div class="modal-body">
                        @lang('Do you want to delete all selected items data?')
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-white" data-bs-dismiss="modal">@lang('Close')</button>
    ...
    <div class="modal fade" id="amenitiesModal" tabindex="-1" aria-labelledby="amenitiesModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Amenities')</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <script>
        $(document).on('ready', function () {
            ...
            HSCore.components.HSDatatables.init($('#datatable'), {
                ...
                ordering: false,
                ajax: {
                    url: "{{ route("admin.all.property.search", ['category' => $category, 'country' => $country, 'state' => $state, 'city' => $city, 'amenity'=>$amenity, 'type'=>$type, 'style'=>$style, 'destination'=>$destination]) }}",
                },
                columns: [
            ...
            $(document).on('click', '#filter_button', function () {
                ...
                datatable.ajax.url("{{ route('admin.all.property.search') }}" + "?filterStatus=" + filterStatus +
            ...
            $(document).on('click', '.delete-multiple', function (e) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('admin.property.delete.multiple') }}",
                    data: {strIds: strIds},
                    datatType: 'json',
                    type: "post",
                    success: function (data) {
                        if (data.success) {
                            Notiflix.Notify.success('All selected Properties have been deleted successfully.');
                            location.reload();
                        }
                        location.reload();
                    },
        ...
        document.addEventListener('DOMContentLoaded', function () {
            $(document).on('click', '.show-amenities', function () {
                const amenitiesData = $(this).data('amenities');
                let html = '';

                if (Array.isArray(amenitiesData) && amenitiesData.length > 0) {
                    amenitiesData.forEach(group => {
                        html += `<div class="mb-3">
                            <h6><span class="badge bg-success text-light">${group.type}</span></h6>
                            <div class="mb-2">`;

                        group.names.forEach(name => {
                            html += `<span class="badge bg-primary text-light me-1 mb-1">${name}</span>`;
                        });
                    ...
                    html = `<div class="text-center text-muted py-4">
                        ...
                        <span>@lang('No amenities available')</span>
                    </div>`;
                }

                $('#amenitiesModalBody').html(html);
