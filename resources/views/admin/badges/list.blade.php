...
@section('page_title',__('Badge List'))
@section('content')
    <div class="content container-fluid">
        <x-page-header menu="Badges" :statBtn="true"/>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <h6 class="card-subtitle mb-3">@lang('Active Badge')</h6>
                                <h3 class="card-title js-counter" data-value="{{ $totalActiveBadges }}">{{ $totalActiveBadges }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $totalBadges }}</span>
                                    <span class="badge bg-soft-success text-success ms-2">
                                        <i class="bi-graph-up"></i> {{ number_format($activeBadgePercentage, 2) }}%
                                    </span>
                                </div>
                            </div>
            ...
            <div class="col-sm-6 col-lg-3 mb-3 mb-lg-4">
                <div class="card card-hover-shadow h-100 stats-card">
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <h6 class="card-subtitle mb-3">@lang('Inactive Badge')</h6>
                                <h3 class="card-title js-counter" data-value="{{ $totalInactiveBadges }}">{{ $totalInactiveBadges }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $totalBadges }}</span>
                                    <span class="badge bg-soft-danger text-danger ms-2">
                                        <i class="bi-graph-up"></i> {{ number_format($inactiveBadgePercentage, 2) }}%
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                ...
                                <h3 class="card-title js-counter" data-value="{{ $totalBadgesThisMonth }}">{{ $totalBadgesThisMonth }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $totalBadges }}</span>
                                    <span class="badge bg-soft-primary text-primary ms-2">
                                        <i class="bi-graph-up"></i> {{ number_format($thisMonthBadgePercentage, 2) }}%
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                ...
                                <h3 class="card-title js-counter" data-value="{{ $totalBadgesThisYear }}">{{ $totalBadgesThisYear }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $totalBadges }}</span>
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <div class="input-group input-group-merge input-group-flush">
                        ...
                        <input id="datatableSearch" type="search" class="form-control" placeholder="Search Badge" aria-label="Search Badge" autocomplete="off">
                    </div>
                </div>
                ...
                <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                    ...
                    <div id="datatableCounterInfo2" class="d-none">
                        <div class="d-flex align-items-center">
                            <a class="btn btn-outline-primary btn-sm inactiveButton" id="inactiveButton" href="javascript:void(0)" data-bs-toggle="modal"
                               data-bs-target="#changeStatusMultiple">
                            ...
                            <div class="offcanvas-body">
                                <form id="filter_form">
                                    <div class="col-12 mb-4">
                                        <span class="text-cap text-body">@lang("Badge")</span>
                                        <input type="text" class="form-control" id="badge" autocomplete="off" value="{{ old('title') }}">
                                    </div>
                                    <div class="col-sm mb-4">
                                        ...
                                        <div class="tom-select-custom">
                                            <select
                                                ...
                                                <option value="1"
                                                        data-option-template='<span class="d-flex align-items-center"><span class="legend-indicator bg-success"></span>@lang('Active')</span>'>
                                                    @lang('Active')
                                                </option>
                                                <option value="0"
                    ...
                    <a href="{{ route('admin.badge.create') }}" class="btn btn-primary btn-sm"><i class="bi-plus-circle me-1"></i>@lang('Add New')</a>
                </div>
            </div>
            ...
            <div class=" table-responsive datatable-custom  ">
                <table id="datatable"
                       class="js-datatable table table-borderless table-thead-bordered table-nowrap table-align-middle card-table text-center"
...
@push('script')
    ...
    <script>
        ...
        $(document).on('click', '.changeStatus', function () {
            let route = $(this).data('route');
            $('.setRouteStatus').attr('action', route);
        })

        $(document).on('click', '.acceptBtn', function () {
            let route = $(this).data('route');
            $('.setRouteAccept').attr('action', route);
        })
        $(document).on('ready', function () {
            ...
            $('.js-sortable').on('change', function () {
                ...
                $('.sortable').each(function (key, val) {
                    let badgeCode = $(val).data('code');
                    badges.push(badgeCode);
                });
                $.ajax({
                    'url': "{{ route('admin.badge.short') }}",
                    'method': "GET",
                    'data': {sort: badges}
                })
            })
            ...
            $(document).on('click', '.delete-multiple', function (e) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('admin.badge.delete.multiple') }}",
                    data: {strIds: strIds},
                    datatType: 'json',
                    type: "post",
                    success: function (data) {
                        if (data.success) {
                            Notiflix.Notify.success('Selected advertisement have been deleted successfully.');
            ...
            $(document).on('click', '.change-status-multiple', function (e) {
                e.preventDefault();
                let all_value = [];
                $(".row-tic:checked").each(function () {
                    all_value.push($(this).attr('data-id'));
                ...
                $.ajax({
                    ...
                    url: "{{ route('admin.badge.status.multiple') }}",
                    data: {strIds: strIds},
                    datatType: 'json',
                    type: "post",
                    success: function (data) {
                        if (data.success) {
                            Notiflix.Notify.success("Selected badge's status changed successfully.");
                            location.reload();
                        }
                        location.reload();
                    },
