...
@section('page_title',__('Affiliate Withdraw Log'))
@section('content')
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <h6 class="card-subtitle mb-3">@lang('Today Withdraw')</h6>
                                <h3 class="card-title js-counter" data-value="{{ $affiliatePayoutRecord[0]['todayWithdraw'] }}">{{ $affiliatePayoutRecord[0]['todayWithdraw'] }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $affiliatePayoutRecord[0]['totalWithdrawLog'] }}</span>
                                    <span class="badge bg-soft-primary text-primary ms-2">
                                        <i class="bi-graph-up"></i> {{ number_format($affiliatePayoutRecord[0]['todayWithdrawPercentage'], 2) }}%
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                ...
                                <h3 class="card-title js-counter" data-value="{{ $affiliatePayoutRecord[0]['successWithdraw'] }}">{{ $affiliatePayoutRecord[0]['successWithdraw'] }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $affiliatePayoutRecord[0]['totalWithdrawLog'] }}</span>
                                    <span class="badge bg-soft-success text-success ms-2">
                                        <i class="bi-graph-up"></i> {{ number_format($affiliatePayoutRecord[0]['successWithdrawPercentage'], 2) }}%
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                ...
                                <h3 class="card-title js-counter" data-value="{{ $affiliatePayoutRecord[0]['pendingWithdraw'] }}">{{ $affiliatePayoutRecord[0]['pendingWithdraw'] }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $affiliatePayoutRecord[0]['totalWithdrawLog'] }}</span>
                                    <span class="badge bg-soft-warning text-warning ms-2">
                                        <i class="bi-graph-up"></i> {{ number_format($affiliatePayoutRecord[0]['pendingWithdrawPercentage'], 2) }}%
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                ...
                                <h3 class="card-title js-counter" data-value="{{ $affiliatePayoutRecord[0]['cancelWithdraw'] }}">{{ $affiliatePayoutRecord[0]['cancelWithdraw'] }}</h3>
                                <div class="d-flex align-items-center">
                                    <span class="d-block fs-6">@lang('from') {{ $affiliatePayoutRecord[0]['totalWithdrawLog'] }}</span>
                                    <span class="badge bg-soft-danger text-danger ms-2">
                                        <i class="bi-graph-up"></i> {{ number_format($affiliatePayoutRecord[0]['cancelWithdrawPercentage'], 2) }}%
                                    </span>
                                </div>
                            </div>
                                ...
                                <form id="filter_form">
                                    <div class="row">
                                        <div class="mb-4">
                                            ...
                                            <div class="row">
                                                <div class="col-12">
                                                    <input type="text" class="form-control" id="transaction_id_filter_input" autocomplete="off" placeholder="@lang('Transaction ID')">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        $(document).on('ready', function () {
            ...
            HSCore.components.HSDatatables.init($('#datatable'), {
                ...
                ordering: false,
                ajax: {
                    url: "{{ route("admin.affiliate.payout.search") }}",
                },
            ...
            document.getElementById("filter_button").addEventListener("click", function () {
                ...
                datatable.ajax.url("{{ route('admin.affiliate.payout.search') }}" + "?filterTransactionID=" + filterTransactionId + "&filterStatus=" + filterStatus + "&filterMethod=" + filterMethod +
...
@push('script')
    ...
    @if (session('success'))
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                Notiflix.Notify.success(@json(session('success')));
            });
        </script>
    @endif
    ...
    @if (session('error'))
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                Notiflix.Notify.failure(@json(session('error')));
            });
        </script>
    @endif
@endpush
