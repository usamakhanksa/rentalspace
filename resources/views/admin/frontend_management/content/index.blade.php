                                        ...
                                        <div class="card-body">
                                            @foreach($singleContent['field_name'] as $name => $type)
                                                <div class="row justify-content-md-between">
                                                    ...
                                                    @endif
                                                    @if($type == "video" && $language->default_status == 1)

                                                        <div class="col-md-4">
                                                            <label class="form-label" for="@lang($name)">@lang(stringToTitle($name))</label>

                                                            <label class="form-check form-check-dashed" for="videoUploader{{$name}}" id="content_video">

                                                                <video id="contentVideo{{$name}}" class="w-100 mb-2" controls
                                                                       style="pointer-events: none;"
                                                                       src="{{ getFile((!$singleContentData->isEmpty()) ? @$singleContentData[$language->id][0]->content->media->{$name}->driver : '',
                                                                                (!$singleContentData->isEmpty()) ? @$singleContentData[$language->id][0]->content->media->{$name}->path : '',
                                                                                true
                                                                           ) }}">
                                                                    @lang('Your browser does not support the video tag.')
                                                                </video>
                                                                <span class="d-block">@lang("Browse your video here")</span>

                                                                <input type="file" name="{{ $name }}"
                                                                       class="form-check-input"
                                                                       id="videoUploader{{$name}}"
                                                                       accept="video/mp4,video/webm,video/ogg"
                                                                       data-hs-file-attach-options='{
                                                                            "textTarget": "#contentVideo{{$name}}",
                                                                            "mode": "video",
                                                                            "targetAttr": "src",
                                                        ...
                                                        <script>
                                                            document.getElementById('videoUploader{{$name}}').addEventListener('change', function(event){
                                                                const file = event.target.files[0];
                                                                if (!file) return;

                                                                const maxSize = 100 * 1024 * 1024;
                                                                if(file.size > maxSize){
                                                                    Notiflix.Notify.failure("File is too large! Maximum allowed size is 100MB.");
                                                                    event.target.value = "";
                                                                    document.getElementById('contentVideo{{$name}}').src = "";
                                                                    return;
                                                                }

                                                                const reader = new FileReader();
                                                                reader.onload = function(e){
                                                                    const video = document.getElementById('contentVideo{{$name}}');
