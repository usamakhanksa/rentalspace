@push('script')
    <script>
        ...
        $('#fileUpload').on('change', function(e) {
            ...
            $.ajax({
                url: '{{ route("affiliate.profile.image.update") }}',
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        $(document).on('click', '.locationData', function () {
            ...
            $.ajax({
                url: '{{ route("affiliate.profile.basic.update") }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    type: typeLower,
        ...
        $('#dynamicBasicSaveBtn').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    currentAnchor.find('.value').text(value);
        ...
        $('[data-bs-target="#dynamicBasicModal"]').on('click', function () {
            ...
            icon = $(this).find('i').attr('class').split(' ')[1]?.replace('fa-', '') || 'edit';
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            saveBtn.addEventListener('click', function () {
                ...
                try {
                    const parsed = JSON.parse(phoneLengthAttr);
                ...
                fetch("{{ route('affiliate.profile.phone.update') }}", {
                    method: "POST",
                    headers: {
