        ...
        <div class="row justify-content-lg-center">
            <div class="col-lg-10 section-payment">
                <div class="personal-info-title listing-top">
                    <div class="text-area">
                        <ul>
                            <li><a href="{{ route('affiliate.profile') }}">@lang('Account')</a></li>
                ...
                <div class="mb-4">
                    @if(!config('withdrawaldays')[date('l')])
                        <h5 class="text-warning alert-text">
                            <i class="fas fa-exclamation-triangle me-2"></i> @lang('Withdraw processing is off today. Please try' )
                            @foreach(config('withdrawaldays') as $key => $days)
                                {{ $days == 1 ? $key.',':'' }}
                            @endforeach
                        </h5>
                    @endif
                ...
                <form action="{{ route('affiliate.payout.request') }}" method="post" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="col-lg-5 col-md-6">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <button type="submit" class="btn-1 w-100 submitBtn mt-3" @disabled(!$open)>
                                            {{ trans('Continue') }}
                                        </button>

                                        <a href="{{ route('affiliate.dashboard') }}" class="btn-2 mt-3 w-100 text-center">{{ trans('Cancel') }}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function supportCurrency(selectedPayoutMethod) {
                ...
                $.ajax({
                    url: "{{ route('affiliate.payout.supported.currency') }}",
            ...
            function checkAmount(amount, selectedCurrency, selectedPayoutMethod) {
                $.ajax({
                    method: "GET",
                    url: "{{ route('affiliate.payout.checkAmount') }}",
                    dataType: "json",
                    data: {
                        'amount': amount,
            ...
            function showCharge(response, currency) {
                ...
                let txnDetails = `<div class="payout-info-box">
                    <ul class="list-group">
